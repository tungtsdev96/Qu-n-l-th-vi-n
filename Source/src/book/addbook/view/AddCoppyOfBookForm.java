...
public class AddCoppyOfBookForm extends javax.swing.JPanel {
    ...
    private void addCoppyOfBookByIdBook() throws ClassNotFoundException, SQLException{
        if (canAdd){
            CoppyOfBook coppyOfBook = new CoppyOfBook();
            for (int i = 0;i<tableAddCoppyOfNewBook.getRowCount();i++){
                coppyOfBook.setIdCoppyOfBook((String) tableAddCoppyOfNewBook.getValueAt(i, 0));
                coppyOfBook.setNumberOfCoopy((int) tableAddCoppyOfNewBook.getValueAt(i, 1));
                coppyOfBook.setPrice((int) tableAddCoppyOfNewBook.getValueAt(i, 4));
                coppyOfBook.setTypeOfCoppy(((boolean)tableAddCoppyOfNewBook.getValueAt(i, 2))?CoppyOfBook.TypeOfCoppy.REFERENCE:CoppyOfBook.TypeOfCoppy.BORROWABLE);
                BookController.getInstance().addCoppyOfBookByIdBook(book.getIdBook(), coppyOfBook);
            }
            notify("Đã thêm");
            lbTitleTable.setText("Thông tin bản sao");
            showInforBook(book.getIdBook());
            displayTableCoppyOfBook(book.getIdBook());
        } else {
            notify("Lỗi .....");
        }
    }
    ...
    private void displayTableCoppyOfBook(String idBook) throws ClassNotFoundException, SQLException {
        lbTitleTable.setText("Thông tin Bản sao");
        book = BookController.getInstance().getBookByIdBook(idBook);
        ArrayList<CoppyOfBook> coppyOfBooks = book.getListCoppyOfBook();
        DefaultTableModel model = (DefaultTableModel) tableAddCoppyOfNewBook.getModel();
        model.setRowCount(0);
        for (int i = 0; i < coppyOfBooks.size(); i++) {
            boolean isReference = true ;
            if (coppyOfBooks.get(i).getTypeOfCoppy() == CoppyOfBook.TypeOfCoppy.REFERENCE) {
                isReference = true;
            } else if (coppyOfBooks.get(i).getTypeOfCoppy() == CoppyOfBook.TypeOfCoppy.BORROWABLE) {
                isReference = false;
            }
            model.addRow(new Object[]{
                coppyOfBooks.get(i).getIdCoppyOfBook(),
                coppyOfBooks.get(i).getNumberOfCoopy(),
                isReference,
                !isReference,
                coppyOfBooks.get(i).getPrice()
            });
        }
        tableAddCoppyOfNewBook.setModel(model);
    }
    ...
    private void showInforBook(String idBook) throws ClassNotFoundException, SQLException {
        ...
        lbNameBook.setText("Tiêu đề: " + book.getTitleBook());
        lbAuthor.setText("Tác Giả: " + book.getNameAuthor());
        lbPublishingCopany.setText("Nhà phát hành: " + book.getPublisher().getNamePublisher());
        lbISBN.setText("ISBN: " + book.getIsbn());
        lbNumberOfCoppy.setText("Số bản sao hiện có: " + book.getListCoppyOfBook().size());
    }
