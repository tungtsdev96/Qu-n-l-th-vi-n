...
//                tfNameAuthor.setText(tfNameAuthor.getText());
    ...
    private void jListSearchBorrowerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jListSearchBorrowerMouseClicked
        ...
        if (i != -1){
            ...
            try {
                tfIdBorower.setText(arrBorrowerOfList.get(i).getIdBorrower());
    ...
    private void binDataToTableBorrowingInformation(Borrower borrower) throws ClassNotFoundException, SQLException{
        ArrayList<BorrowingInformation> arr = BorrowingInformationController.getInstance().getListBorrowingInformationByIdBorrower(borrower);
        DefaultTableModel model = (DefaultTableModel) tableBorrowingInformation.getModel();
        model.setRowCount(0);
        for (int i = 0;i<arr.size();i++){
            BorrowingInformation brIngor = arr.get(i);
            String status = null;
            if (brIngor.getBorrowStatus() == BorrowingInformation.BorrowStatus.BORROWED){
                status = "Đã đăng kí";
            } else if (brIngor.getBorrowStatus() == BorrowingInformation.BorrowStatus.LENDING){
                status = "Đang mượn";
            } else {
                status = "Đã trả";
            }
            model.addRow(new Object[]{
                brIngor.getIdBorrowingInformation(),
                brIngor.getBorrowedDate(),
                brIngor.getExpectReturnDate(),
                status,
                brIngor.getDeposit()
            });
        }
    ...
    private void binDataToTableDetailBorrowingInformation(String idBorrowerInformation) throws ClassNotFoundException, SQLException{
        ArrayList<DetailLentBook> arr = DetailLentBookController.getInstance().getListDetailLentBookByIdBorrowInfor(idBorrowerInformation);
        DefaultTableModel model = (DefaultTableModel) tableDetailBorrowingInfor.getModel();
        model.setRowCount(0);
        for(DetailLentBook b:arr){
           String idBook = BookHelper.getIdBookByIdCoppyOfBook(b.getIdCoppyOfBook());
           Book book = BookController.getInstance().getBookByIdBook(idBook);
           String status = null;
           if (b.getStatus()==0){
               status = "Đăng kí";
