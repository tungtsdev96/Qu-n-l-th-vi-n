...
import borrowbook.controller.RegistrationBookController;
...
public class RegistrationOfBorrowingBookForm extends javax.swing.JFrame {
     ...
     *
     */
    RegistrationBookController controller;
    Borrower borrower;
    DetailLentBookController detail;

    public RegistrationOfBorrowingBookForm(Borrower borrower) throws ClassNotFoundException, SQLException {
        initComponents();
        setLocationRelativeTo(this);
        detail = new DetailLentBookController();
        controller = new RegistrationBookController();
    ...
    public void setSelectedRow() throws ClassNotFoundException, SQLException {
        ...
        // idBook;
            String idBook = tableBookSearching.getValueAt(i, tableBookSearching.getColumn("Mã sách").getModelIndex()).toString();
                ...
                putIntoDetailTable(BookController.getInstance().getBookByIdBook(idBook), tableBookDetail);
    }
    ...
    public void putIntoDetailTable(Book book, JTable table) {
        ...
        for (int j = 0; j < book.getListCoppyOfBook().size(); j++) {
            CoppyOfBook coppyOfBook = book.getListCoppyOfBook().get(j);
            Vector vec = new Vector();
            vec.add(book.getListCoppyOfBook().get(j).getIdCoppyOfBook());

            String typeOfCoppy = "";
            if (coppyOfBook.getTypeOfCoppy().toString() == "REFERENCE") {
                typeOfCoppy = "Tài liệu tham khảo";
            }
            if (coppyOfBook.getTypeOfCoppy().toString() == "BORROWABLE") {
            ...
            if (coppyOfBook.getStatusOfCoppy().toString() == "AVAILABLE") {
                statusofCoppy = "Có sẵn";
            }
            if (coppyOfBook.getStatusOfCoppy().toString() == "BORROWED") {
                statusofCoppy = "Đã được đăng ký";
            }
            if (coppyOfBook.getStatusOfCoppy().toString() == "LENT") {
...
//        }
    ...
    private void checkConditionBtnSendRequest() {
        ...
        try {
            // TODO add your handling code here:
            if (controller.checkRegistrationFromBorrower(borrower.getIdBorrower())) {
                pushNotice("Hủy đăng ký cũ trước khi đăng ký mới");
                return;
            }

            if (controller.checkBorrowingCard(controller.getBorrwer(borrower.getIdBorrower()))) {
                int numberOfRow = tableChoosingList.getRowCount();

                if (numberOfRow != 0) {
                    if (controller.checkNumberBook(borrower.getIdBorrower(), numberOfRow)) {
                        ...
                        for (int i = 0; i < numberOfRow; i++) {
                            ...
                            if (i == 0) {
                                maThongTinMuonTra = controller.addInfoBorrowing(controller.getBorrwer(borrower.getIdBorrower()));
                            }
    ...
    private void checkConditionBtnAdd() {
        if (tableBookSearching.getSelectedRow() != -1 && tableBookDetail.getSelectedRow() != -1) {
            ...
            if (loaiSach == "Tài liệu tham khảo" || trangThai != "Có sẵn") {
                ...
                try {
                    try {
                        if (controller.checkIdBook(borrower.getIdBorrower(), maSach)) {
