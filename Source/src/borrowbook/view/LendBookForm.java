...
import borrowbook.view.BorrowerInforHelper;
import book.addbook.view.BookHelper;
import book.controller.BookController;
import user.controller.BorrowerController;
import borrowbook.controller.BorrowingInformationController;
import borrowbook.controller.DetailLentBookController;
...
//        mapBorrowingInfor = new HashMap<>();
        try {
            arrAvailableBorrower = BorrowerController.getInstance().getListBorrower();
...
//            }
    ...
    private void searchBorrower() throws SQLException, ClassNotFoundException {
        ...
        ArrayList<Borrower> list_borrower = BorrowerController.getInstance().searchBorrower(key);
    ...
    private void update() throws ClassNotFoundException, SQLException {
        ...
        for (int row = 0; row < tableBorrowingInformation.getRowCount(); row++) {
            ...
            CoppyOfBook coppyOfBook = BookController.getInstance().getCoppyOfBookById(idCoppyBook);
            CoppyOfBook.StatusOfCoppy statusOfCoppy = (boolean) tableBorrowingInformation.getValueAt(row, 5) ? CoppyOfBook.StatusOfCoppy.LENT : CoppyOfBook.StatusOfCoppy.AVAILABLE;
            coppyOfBook.setStatusOfCoppy(statusOfCoppy);
            
            if (statusOfCoppy == CoppyOfBook.StatusOfCoppy.LENT) {
                fineBorrower += (int) tableBorrowingInformation.getValueAt(row, 3);
            } else {
                DetailLentBookController.getInstance().deleteDetailLentBook(idBorrowInfor, idCoppyBook);
            }
            
            //update status book
            BookController.getInstance().updateCoppyOfBook(coppyOfBook);
            
            //update status detail 
            DetailLentBook detailLentBook = new DetailLentBook();
            detailLentBook.setIdCoppyOfBook(idCoppyBook); 
            detailLentBook.setLentDate(lentDate); 
            detailLentBook.setStatus(1);
            DetailLentBookController.getInstance().updateDetailLentBook(idBorrowInfor, detailLentBook);
        }

       //update infor borrow
        BorrowingInformation borrowingInformation = new BorrowingInformation();
        borrowingInformation.setIdBorrowingInformation(idBorrowInfor);
        borrowingInformation.setExpectReturnDate(expectReturnDate); 
        borrowingInformation.setDeposit(fineBorrower); 
        boolean is = BorrowingInformationController.getInstance().updateLent(borrowingInformation);
    ...
    private void updateWhentNotLentAllBook() throws ClassNotFoundException, SQLException {
        ...
        for (int row = 0; row < tableBorrowingInformation.getRowCount(); row++) {
            idBorrowInfor = (String) tableBorrowingInformation.getValueAt(row, 0);
            String idCoppyBook = (String) tableBorrowingInformation.getValueAt(row, 1);
            CoppyOfBook b = BookController.getInstance().getCoppyOfBookById(idCoppyBook);
            b.setStatusOfCoppy(CoppyOfBook.StatusOfCoppy.AVAILABLE);
            DetailLentBookController.getInstance().deleteDetailLentBook(idBorrowInfor, idCoppyBook);
            BookController.getInstance().updateCoppyOfBook(b);
        }
        BorrowingInformationController.getInstance().deleteBorrowingInfor(idBorrowInfor);
        binDataToTableBorrowedInformation(borrower);
    ...
    private void showInforBorrower(Borrower borrower) throws ClassNotFoundException, SQLException {
        lbIdBorrowCard.setText("Mã thẻ: " + borrower.getBorrowingCard().getIdBorrowingCard());
        lbIdBorrower.setText("Mã người mượn: " + borrower.getIdBorrower());
        lbNameBorrower.setText("Tên người mượn: " + borrower.getName());
        tfExpiredDate.setText("Ngày hết hạn: " + borrower.getBorrowingCard().getIdBorrowingCard());
        binDataToTableBorrowedInformation(borrower);
    }
    ...
    private DefaultTableModel getModelBorowed(Borrower borrower) throws ClassNotFoundException, SQLException {
        ...
        ArrayList<BorrowingInformation> arrBorrowInfor = BorrowingInformationController.getInstance().getListBorrowingInformationByIdBorrower(borrower);
        for (BorrowingInformation b : arrBorrowInfor) {
            if (b.getBorrowStatus() != BorrowingInformation.BorrowStatus.BORROWED) {
                continue;
            }
            ArrayList<DetailLentBook> arr = DetailLentBookController.getInstance().getListDetailLentBookByIdBorrowInfor(b.getIdBorrowingInformation());
            for (DetailLentBook detailLentBook : arr) {
                Book book = BookController.getInstance().getBookByIdCoppyOfBook(detailLentBook.getIdCoppyOfBook());
                CoppyOfBook coppyOfBook = BookController.getInstance().getCoppyOfBookById(detailLentBook.getIdCoppyOfBook());
                model.addRow(new Object[]{
                    b.getIdBorrowingInformation(),
                    detailLentBook.getIdCoppyOfBook(),
                    book.getTitleBook(),
                    coppyOfBook.getPrice(),
                    b.getBorrowedDate(),
                    false,
                    false
                });
    ...
    private DefaultTableModel getModelLending(Borrower borrower) throws ClassNotFoundException, SQLException {
        ...
        ArrayList<BorrowingInformation> arrBorrowInfor = BorrowingInformationController.getInstance().getListBorrowingInformationByIdBorrower(borrower);
        for (BorrowingInformation b : arrBorrowInfor) {
            if (b.getBorrowStatus() != BorrowingInformation.BorrowStatus.LENDING) {
                continue;
            }
            ArrayList<DetailLentBook> arr = DetailLentBookController.getInstance().getListDetailLentBookByIdBorrowInfor(b.getIdBorrowingInformation());
            for (DetailLentBook detailLentBook : arr) {
                ...
                model.addRow(new Object[]{
                    ...
                    coppyOfBook.getPrice(),
                    b.getBorrowedDate(),
                    detailLentBook.getLentDate(),
                    b.getExpectReturnDate()
                });
            }
        }
        return model;
    ...
    private void initListSearchBorrower(ArrayList<Borrower> arr) {
        ...
        for (int i = 0; i < arrBorrowerOfList.size(); i++) {
            listModel.addElement(arrBorrowerOfList.get(i).getIdBorrower() + ": " + arrBorrowerOfList.get(i).getName());
        }
    ...
    private void autoComplete(String kt) {
        ...
        for (i = 0; i < arrAvailableBorrower.size(); i++) {
            if (arrAvailableBorrower.get(i).getIdBorrower().startsWith(kt)) {
